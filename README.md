...
- **Current market size + growth** (2025–2026), with the 2022 IBISWorld figure kept as a labeled baseline so the shift is visible
- **What's happening now** — a deep read on the live dynamics (disruption, M&A, regulation, competitive shifts)
...
- **Written by:** Claude (Haiku only) — one research agent per industry, plus a synthesis pass for the cross-cutting trends.
- **Data files:** `briefs_full.json` (per-industry data), `trends_full_raw.json` (the trend synthesis).

## Caveat

Figures carry their year and their source. Market-size estimates vary by scope across research providers — verify against the cited source before relying on any single number.
