...
def render_memo(theme: dict, prefix: str = "") -> str:
    ...
    industry_cards = "".join(brief_card(item) for item in theme["subthemes"][0]["industries"][:2]) if theme["subthemes"] else ""
    seen_industries = set()
    industry_rows = []
    for subtheme in theme["subthemes"]:
        for industry in subtheme["industries"]:
            slug = industry.get("slug")
            if slug and slug not in seen_industries:
                seen_industries.add(slug)
                industry_rows.append(industry)
    industry_cards = "".join(brief_card(item) for item in industry_rows[:4])
    company_chips = []
    seen_companies = set()
    for subtheme in theme["subthemes"]:
        for company in subtheme["companies"]:
            ...
            if slug and slug not in seen_companies:
                ...
                company_chips.append(company_link(company, prefix=prefix))
  ...
  <div class="split">
    ...
    <div class="panel">
      ...
      <div class="chips">{representative_companies}</div>
    </div>
  </div>
  <div class="split">
    <div class="panel">
